...
5. **目录与变更：** 遵守 **`references/rules/storage.md`** 与 **`references/rules/change-and-versioning.md`**（归档流程）。
6. **OpenSpec（可选）：** 若业务仓库使用 OpenSpec，变更与校验方式遵循该仓库约定。
...
5. **图示（draw.io）：** `diagram-skill`（泳道、流程、架构、时序等；YAML→CLI→`.drawio`）
